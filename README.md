...
- **Dual-Tier AI Routing:** Serverless routing through **Cerebras (Qwen 3 235B)** for blazing-fast inference, with automatic fallback to **Groq (Llama 3.3 70B)** during rate limits. Custom API keys route directly to **Gemini 3 Flash**.
- **Proactive Search Grounding (RAG):** Live web search via **Tavily API** or **Brave Search API** whenever research, news, or source compilations are requested.

---

## 🛠️ Tech Stack

- **Frontend:** Vanilla HTML5, CSS3 (Dark Mode, Gold Ambient Glows, Glassmorphism, Responsive Grid), Custom JavaScript Core.
...
- **Search Retrieval:** Tavily Search API / Brave Search API.
- **AI Inference:** Cerebras LPU & Groq Cloud (server-side), Google Gemini 3 Flash (client-side optional).
...
Configure [config.js](config.js) in the root of the project:
```javascript
const CONFIG = {
...
2. **Start the local dev server:**
   ...
   npm run dev
   ```
   *Spins up Wrangler Pages dev proxy at `http://localhost:8789`.*

3. **Verify functions compile:**
   ```bash
   npm run build
   ```
